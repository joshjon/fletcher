...
auto-fixing. Re-run after each change to confirm.`,
		Flags: []cli.Flag{
			...
			&cli.IntFlag{
				Name:    "wireguard-port",
				Usage:   "UDP port to probe for router forwarding (matches the daemon's listen port)",
				Sources: cli.EnvVars("FLETCHER_WIREGUARD_PORT"),
				Value:   51820,
			},
			&cli.IntFlag{
				Name:    "pairing-port",
				Usage:   "TCP port to probe for router forwarding (the iOS pairing port)",
		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			checkers := []doctor.Checker{
				...
				doctor.CheckJobRuntime(cmd.String("socket")),
				doctor.CheckBaseImage(cmd.String("socket")),
				doctor.CheckDefaultRoutes(),
				doctor.CheckPublicIP(),
				doctor.CheckPortMapping(cmd.Int("wireguard-port"), cmd.Int("pairing-port")),
				doctor.CheckPublicEndpoint(cmd.String("socket")),
				doctor.CheckPairingEndpoint(cmd.String("socket")),
