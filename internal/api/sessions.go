...
type SessionsBackend interface {
	Create(ctx context.Context, name, image, egressPolicy, gateway string) (session.Session, error)
...
func (s *SessionsService) CreateSession(ctx context.Context, req *connect.Request[fletcherv1.CreateSessionRequest]) (*connect.Response[fletcherv1.CreateSessionResponse], error) {
	sess, err := s.backend.Create(ctx, req.Msg.GetName(), req.Msg.GetImage(), req.Msg.GetEgressPolicy(), req.Msg.GetGateway())
	if err != nil {
		return nil, err
	}
