...
This is **opt-in**. It opens ports 80/443 on your box, the only thing besides the
silent WireGuard port that faces the internet, so it stays off until you turn it
on. It needs a real public IP (not behind CGNAT) with 80/443 reachable.
...
since it already knows your public IP. Add that record at your DNS provider.
