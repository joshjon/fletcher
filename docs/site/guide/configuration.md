...
settings` - no editing systemd unit files. Settings take effect on the next
daemon start.
...
Only **bootstrap** config - where the database, socket, and age key live - stays
in the flag/env layer, because it's needed to open the database these settings
live in. Everything else is a setting.
